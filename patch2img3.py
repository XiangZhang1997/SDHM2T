...
def merge_patches_3(patches, volume_size, overlap_size):

    height, width, channels = volume_size
    patch_height, patch_width, _ = patches.shape[1:]
    ...
    merged_volume = np.zeros((height, width, channels), dtype=np.float32)
    weight_volume = np.zeros((height, width, channels), dtype=np.float32)
    ...
    for y in range(num_patches_y):
        for x in range(num_patches_x):
            ...
            merged_volume[y_start:y_start+patch_height, x_start:x_start+patch_width, :] += patches[idx]
            weight_volume[y_start:y_start+patch_height, x_start:x_start+patch_width, :] += 1
            idx += 1

    for i in range(weight_volume.shape[0]):
        for j in range(weight_volume.shape[1]):
            for k in range(weight_volume.shape[2]):
                if weight_volume[i][j][k] == 0.0 or weight_volume[i][j][k] == 0:
                    weight_volume[i][j][k] = 1e-10

    weight_volume[weight_volume == 0] = 1e-10  
...
def merge_patches(patches, volume_size, overlap_size):
    ...
    for x in range(num_patches_x):
        for y in range(num_patches_y):
            ...
            # print(weight_volume[x_start:x_start+patch_width, y_start:y_start+patch_height])
            idx += 1
    for i in range(weight_volume.shape[0]):
        for j in range(weight_volume.shape[1]):
...
def p2i_3(patch_L,w=565,h=584,overlap_w=0,overlap_h=0):

    patch_L = np.array(patch_L)
    newimg = merge_patches_3(patch_L,(w,h,patch_L.shape[-1]),(overlap_w, overlap_h))
    return newimg
